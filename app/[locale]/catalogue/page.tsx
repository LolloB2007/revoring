    ...
    <section className="container-x py-16 md:py-24">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <AdminEditLink href="/admin/products/new" label={locale === "it" ? "Nuovo prodotto" : "New product"} />
          <AdminEditLink href="/admin/products" label={locale === "it" ? "Gestisci" : "Manage"} />
        </div>
      </div>
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p) => (
            <div key={p.id} className="group block relative">
              <AdminEditLink href={`/admin/products/${p.id}`} variant="card" />
              <Link href={`/${locale}/catalogue/${p.slug}`}>

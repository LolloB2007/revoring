/**
 * Tiny JSON-file-backed data store. One JSON file per table, kept in `data/`.
 *
 * Design notes:
 * - Per-table in-process mutex serialises writes so concurrent server actions
 *   can't clobber each other (good enough for a small Node server).
 * - Reads are atomic at the JSON-parse level — the worst case is a microsecond
 *   of stale data, which is fine for this product.
 * - Atomic writes: write to `<file>.tmp` then rename, so a crash mid-write
 *   doesn't truncate the canonical file.
 *
 * Limitations:
 * - NOT safe on Vercel serverless (filesystem is read-only at runtime, /tmp is
 *   ephemeral per instance). Works on any persistent host: local dev,
 *   self-hosted Node, Fly, Railway, Render, a $5 VPS, etc.
 * - No indexes; linear scans. Fine up to thousands of rows per table.
 */
